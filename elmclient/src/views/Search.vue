  ...
  setup() {
    ...
    onMounted(() => {
      ...
      if (user.value && user.value.userId) {
        try {
          const savedHistory = localStorage.getItem(`search_history_${user.value.userId}`);
          if (savedHistory) {
    ...
    const performSearch = async () => {
      ...
      try {
        // 使用BusinessController/listBusinessByOrderTypeId/1获取所有商家
        const response = await axios.get('BusinessController/listBusinessByOrderTypeId/1');
        
        if (response.data && response.data.result) {
          // 在前端进行过滤，查找名称中包含搜索关键词的商家
          const query = searchQuery.value.toLowerCase();
          searchResults.value = response.data.result.filter(business => 
            business.businessName && business.businessName.toLowerCase().includes(query)
          );
          
          // 保存搜索历史到本地存储
          if (user.value && user.value.userId) {
            let savedHistory = localStorage.getItem(`search_history_${user.value.userId}`);
            let historyArray = savedHistory ? JSON.parse(savedHistory) : [];
            
            // 避免重复添加
            if (!historyArray.includes(searchQuery.value)) {
              historyArray.unshift(searchQuery.value); // 添加到历史开头
              // 最多保存10条历史记录
              historyArray = historyArray.slice(0, 10);
              localStorage.setItem(`search_history_${user.value.userId}`, JSON.stringify(historyArray));
