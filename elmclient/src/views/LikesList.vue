      ...
      try {
        ...
        for (const business of businesses) {
          try {
            ...
            const likeResponse = await axios.post('LikesController/getLikesByUserIdByBusinessId', likeData);
            
            // 如果返回1，表示用户已点赞
            if (likeResponse.data && likeResponse.data.result === 1) {
              likedBusinesses.push({
                businessId: business.businessId,
                name: business.businessName,
                img: business.businessImg || 'data:image/gif;base64,R0lGODlhAQABAIAAAMLCwgAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==',
                type: 'business',
                likeTime: new Date().toISOString() // 由于API没有返回点赞时间，使用当前时间
              });
            }
          } catch (error) {
            console.error(`检查商家${business.businessId}点赞状态失败:`, error);
          }
        }
        
        likesList.value = likedBusinesses;
      } catch (error) {
