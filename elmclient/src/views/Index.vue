<template>
    ...
    <div class="wrapper">
        ...
        <!-- 积分商城入口 -->
        <div class="credit-mall" @click="navigateToCredit">
            <div class="left">
...
export default {
    ...
    setup() {
        ...
        const getBusinessList = () => {
            axios.get(`BusinessController/listBusinessByOrderTypeId/1`)
                .then(response => {
                    // 处理返回数据结构
                    if (response.data && response.data.result) {
                        businessList.value = response.data.result;
                    } else if (Array.isArray(response.data)) {
                        businessList.value = response.data;
                    } else {
                        console.error('获取商家列表数据格式不正确:', response.data);
                        businessList.value = [];
                    }
                    
                    // 打印调试信息
                    console.log('获取到的商家列表:', businessList.value);
                })
                .catch(error => {
...
.wrapper .credit-mall:active {
    background-color: #D9EFFF;
}

.wrapper .credit-mall .left {
    display: flex;
    align-items: center;
    margin-left: 4vw;
    user-select: none;
...
.wrapper .credit-mall .left i {
    font-size: 6vw;
    margin-right: 2vw;
    color: #0097FF;
}

.wrapper .credit-mall .left h3 {
    font-size: 4vw;
    margin-right: 2vw;
}

.wrapper .credit-mall .left p {
    font-size: 3vw;
}

.wrapper .credit-mall .right {
    font-size: 3vw;
    margin-right: 4vw;
    cursor: pointer;
}
